...
//代码在init_filter执行过程中，返回值小于0，怀疑是输出文件的原因
...
static int init_filters(void) {
	...
	unsigned int i;
	int ret;
	filter_ctx = (FilteringContext *) av_malloc_array(ifmt_ctx->nb_streams,
			sizeof(*filter_ctx));
	if (!filter_ctx)
...
JNIEXPORT jlong JNICALL Java_com_example_ffmpeg_MainActivity_ffmpeg_1mp4_1mkv(
	...
	const char* in_file = "/storage/sdcard0/ffmpeg/cuc_ieschool.ts";
	const char* out_file = "/storage/sdcard0/ffmpeg/in.avi";
