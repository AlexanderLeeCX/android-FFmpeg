...
int flush_encoder(AVFormatContext *fmt_ctx, unsigned int stream_index) {
	...
	while (1) {
		...
		ret = avcodec_encode_audio2(fmt_ctx->streams[stream_index]->codec,
...
JNIEXPORT jlong JNICALL Java_com_example_ffmpeg_MainActivity_ffmpeg_1pcm_1aac(
	...
	const char* out_file = "/storage/sdcard0/ffmpeg/tdjm.aac";
	int i;
	in_file = fopen("/storage/sdcard0/ffmpeg/tdjm.pcm", "rb");
	...
	LOGV("pCodecCtx = audio_st->codec;");
	pCodecCtx->codec_id = fmt->audio_codec;
	LOGV("pCodecCtx->codec_id = fmt->audio_codec;");
